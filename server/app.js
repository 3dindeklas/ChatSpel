...
function resolveSessionTimeoutMs(value) {
...
function createApp(options = {}) {
  ...
  const sessionTimeoutMs = resolveSessionTimeoutMs(
    options.sessionTimeoutMs ?? process.env.SESSION_TIMEOUT_MS ?? 60000
  );
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      if (!sanitizedOptions.length) {
        res
          .status(400)
          .json({ message: "Voeg minimaal één antwoordoptie toe" });
        return;
      }
      ...
      if (!hasCorrect) {
        res
          .status(400)
          .json({ message: "Markeer minimaal één juist antwoord" });
        return;
      }
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      const cutoff = new Date(Date.now() - sessionTimeoutMs).toISOString();
